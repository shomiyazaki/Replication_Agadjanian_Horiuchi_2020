...
```{r}
he11 <- plot_models(ext_h1_country, ext_h1_policy, ext_h1_people, ext_h1_trump,
            rm.terms = c("2. favorable|1. most favorable",
                         "3. neutral|2. favorable",
                         "4. unfavorable|3. neutral",
                         "5. most unfavorable|4. unfavorable"),
            vline.color = "gold")+
  labs(title = paste("Odds Ratios of the Coefficients"),
       subtitle = paste("Note: The point range is 95% condidence intervals"),
       x = "Predictors",
       y = "Odds Ratio")
he11
```
...
he12 <- ggplot(data = plot_tab, aes(x = vars, y = est, color = model)) +
  ...
  coord_flip()+
  scale_colour_manual(values = c("purple", "mediumseagreen", "dodgerblue", "firebrick1"),
                      breaks = c("ext_h1_country", "ext_h1_people", "ext_h1_policy", "ext_h1_trump"),
                      labels = c("countryfav", "peoplefav", "policyfav", "trumpfav"))+
  labs(title = paste("Interval between the Thresholds of Likert Scale"),
       subtitle = paste("Note: The point range is 99% condidence intervals"),
       x = "Cutoffs of Likert Scale",
       y = "Estimated Coefficients")+
  geom_hline(yintercept = 0, col = "gold", lwd = 1)
...
extlmh22 <- lm(compfav ~ source * issue, data = df_ext)
h1plot <- plot_model(extlmh21, type = "int") + ylim(2.7, 3.3)
h2plot <- plot_model(extlmh22, type = "int") + ylim(2.7, 3.3)
...
```{r}
he21 <- plot_model(ext_h2_country1, type = "int")
he22 <- plot_model(ext_h2_people1, type = "int")
he23 <- plot_model(ext_h2_policy1, type = "int")
he24 <- plot_model(ext_h2_trump1, type = "int")

he21$facet$params$nrow <- 1
he22$facet$params$nrow <- 1
he23$facet$params$nrow <- 1
he24$facet$params$nrow <- 1
...
```{r}
he31 <- plot_model(ext_h2_country2, type = "int")
he32 <- plot_model(ext_h2_people2, type = "int")
he33 <- plot_model(ext_h2_policy2, type = "int")
he34 <- plot_model(ext_h2_trump2, type = "int")

he31$facet$params$nrow <- 1
he32$facet$params$nrow <- 1
he33$facet$params$nrow <- 1
he34$facet$params$nrow <- 1
...
rm(list= ls()[!(ls() %in% c("all", 
                            ...
                            "ext_h2_people2",
                            "ext_h2_trump2"))])
save.image("tablesfigures.Rdata")
```
